package testingil.webinar.tddapi.s07.ut_passing;
...
class NavigatorAPITests {
	...
	@BeforeEach
	public void setup() {
		this.mockMvc = MockMvcBuilders.standaloneSetup(new NavigationController7()).build();
	}
	
	private void setDestination(Location location) throws JsonProcessingException, Exception {
		mockMvc.perform(post("/navigator7/destination")
	...
	private void setStartPoint(Location location) throws Exception {
		mockMvc.perform(post("/navigator7/startpoint")
				.contentType(MediaType.APPLICATION_JSON)
				.content(toJson(location)))
        .andExpect(status().isOk());
	}
	...
	private int distanceToDestination() throws JsonProcessingException, Exception {
		MvcResult result = 
			mockMvc.perform(get("/navigator7/distance"))
	        .andExpect(status().isOk())
	        .andReturn();
