package testingil.webinar.tddapi.s17.fill_controller;
...
import testingil.webinar.tddapi.s17.Distance;
import testingil.webinar.tddapi.s17.Location;
import testingil.webinar.tddapi.s17.NavigationController17;
...
class NavigatorAPITests {
	...
	@BeforeEach
	public void setup() {
		this.mockMvc = MockMvcBuilders.standaloneSetup(new NavigationController17()).build();
	}
	
	private void setDestination(Location location) throws JsonProcessingException, Exception {
		mockMvc.perform(post("/navigator17/destination")
	...
	private void setStartPoint(Location location) throws Exception {
		mockMvc.perform(post("/navigator17/startpoint")
				.contentType(MediaType.APPLICATION_JSON)
				.content(toJson(location)))
        .andExpect(status().isOk());
	}
	...
	private int distanceToDestination() throws JsonProcessingException, Exception {
		MvcResult result = 
			mockMvc.perform(get("/navigator17/distance"))
	        .andExpect(status().isOk())
	        .andReturn();
