...
{
    [TestClass]
    public class NavigationAPITests_S03
    {
        [TestMethod]
		...
		private async Task setDestination(Location location)
		{
			...
			var response = await client.PostAsync("/navigator/destination", content);
		...
		private async Task setStartPoint(Location location)
		{
			var content = new StringContent(JsonConvert.SerializeObject(location),
				Encoding.UTF8, "application/json");
			var response = await client.PostAsync("/navigator/startpoint", content);
			Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
		}
		...
		{
			var response = await client.GetAsync("/navigator/distance");
			Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
