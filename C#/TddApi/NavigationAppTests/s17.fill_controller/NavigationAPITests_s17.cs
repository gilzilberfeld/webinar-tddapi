...
namespace NavigationApp.s17.fill_controller
{
    ...
    public class NavigationAPITests_s18
    {
		...
		private async Task setDestination(Location location)
		{
			...
			var response = await client.PostAsync("/navigator17/destination", content);
		...
		private async Task setStartPoint(Location location)
		{
			var content = new StringContent(JsonConvert.SerializeObject(location),
				Encoding.UTF8, "application/json");
			var response = await client.PostAsync("/navigator17/startpoint", content);
			Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
		}
		...
		{
			var response = await client.GetAsync("/navigator17/distance");
			Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
